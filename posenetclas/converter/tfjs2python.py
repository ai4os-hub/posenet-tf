...
def load_variables(chkpoint, base_dir=BASE_DIR):
    ...
    for x in variables:
        ...
        byte = open(
            os.path.join(base_dir, chkpoint, filename), "rb"
        ).read()
...
def build_network(image, layers, variables):
    ...
    def _depthwise_weights(layer_name):
        return variables[
            "MobilenetV1/" + layer_name + "/depthwise_weights"
        ]["x"]

    def _conv_to_output(mobile_net_output, output_layer_name):
        w = tf.nn.conv2d(
            mobile_net_output,
            _weights(output_layer_name),
            [1, 1, 1, 1],
            padding="SAME",
        )
        w = tf.nn.bias_add(
            w, _biases(output_layer_name), name=output_layer_name
        )
        return w
    ...
    def _separable_conv(inputs, stride, block_id, dilations):
        ...
        w = tf.nn.conv2d(
            w, _weights(pw_layer), [1, 1, 1, 1], padding="SAME"
        )
    ...
    with cg.as_default():
        ...
        with tf.Session() as sess:
            ...
            checkpoint_path = saver.save(
                sess, save_path, write_state=False
            )

            tf.train.write_graph(
                cg, model_dir, "model-%s.pbtxt" % chkpoint
            )
