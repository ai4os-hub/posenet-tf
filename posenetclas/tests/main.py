...
def test_predict_data():
    ...
    for content_type in [
        ...
        fpath = os.path.join(
            paths.get_base_dir(), "data", "samples", "runner.jpg"
        )
        tmp_fpath = os.path.join(
            paths.get_base_dir(), "data", "samples", "tmp_file.jpg"
        )
        copyfile(
            fpath, tmp_fpath
        )  # copy to tmp because we are deleting the file after prediction
        file = UploadedFile(
            name="data", filename=tmp_fpath, content_type="image/jpg"
        )
...
def test_curl_predict_url():
    ...
    if output_type != content_type:
        raise Exception(
            "Output type {} is different from expected {}".format(
