...
def within_nms_radius(poses, squared_nms_radius, point, keypoint_id):
    for _, _, pose_coord in poses:
        if (
            np.sum((pose_coord[keypoint_id] - point) ** 2)
            <= squared_nms_radius
        ):
            return True
    return False
...
def within_nms_radius_fast(pose_coords, squared_nms_radius, point):
    if not pose_coords.shape[0]:
        return False
    return np.any(
        np.sum((pose_coords - point) ** 2, axis=1)
        <= squared_nms_radius
    )
...
def get_instance_score_fast(
    ...
    if exist_pose_coords.shape[0]:
        ...
        not_overlapped_scores = np.sum(
            keypoint_scores[np.all(s, axis=0)]
        )
    else:
...
def build_part_with_score_fast(
    ...
    for keypoint_id in range(num_keypoints):
        ...
        max_vals = ndi.maximum_filter(
            kp_scores, size=lmd, mode="constant"
        )
...
def decode_multiple_poses(
    ...
    pose_keypoint_scores = np.zeros(
        (max_pose_detections, NUM_KEYPOINTS)
    )
    pose_keypoint_coords = np.zeros(
        (max_pose_detections, NUM_KEYPOINTS, 2)
    )
    ...
    scored_parts = sorted(
        scored_parts, key=lambda x: x[0], reverse=True
    )
