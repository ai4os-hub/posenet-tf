...
def get_predict_args():
    ...
    parser['urls'] = fields.String(required=False,
                                   load_default=None,
                                   metadata={"description": "Select an URL of the image you want to classify."})
    # missing action="append" --> append more than one url

    # Add format type of the response
    parser['accept'] = fields.Str(
        metadata={"description": "Media type(s) that is/are acceptable for the response."},
        load_default='application/zip',
