...
def load_model(model_id, sess, model_dir=MODEL_DIR):
    ...
    model_path = os.path.join(
        model_dir, "model-%s.pb" % model_cfg["checkpoint_name"]
    )
    if not os.path.exists(model_path):
        print(
            "Cannot find model file %s, converting from tfjs..."
            % model_path
        )
    ...
    displacement_fwd = sess.graph.get_tensor_by_name(
        "displacement_fwd_2:0"
    )
    displacement_bwd = sess.graph.get_tensor_by_name(
